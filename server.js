...
const ensureSupabase = (res) => {
  ...
  res.status(500).json({ error: "Supabase non configuré. Vérifie SUPABASE_URL/ANON/SERVICE_KEY." });
  return false;
};
...
const uploadToSupabase = async ({ image, userId }) => {
  ...
  const { error } = await supabaseAdmin.storage.from(supabaseBucket).upload(fileName, buffer, {
...
const insertImageRow = async ({ image, storagePath, userId, mode, prompt, projectId }) => {
  const { data, error } = await supabaseAdmin
    .from("images")
    .insert({
    ...
    .select("id, created_at, storage_path, mime_type, created_by, mode, prompt, project_id")
    .single();
  if (error) throw error;
  return data;
};
...
const normalizeImageConfig = (config) => {
  ...
  const imageSizeInput =
    typeof config.imageSize === "string" ? config.imageSize.trim().toUpperCase() : "";
  const aspectRatioInput =
    typeof config.aspectRatio === "string" ? config.aspectRatio.trim() : "";

  const imageSize = VALID_IMAGE_SIZES.has(imageSizeInput) ? imageSizeInput : "4K";
  const aspectRatio = VALID_ASPECT_RATIOS.has(aspectRatioInput) ? aspectRatioInput : null;
...
app.get("/api/library", requireAuth, requireActiveProfile, async (req, res) => {
  ...
  if (error) {
    return res.status(500).json({ error: "Impossible de charger la bibliothèque." });
  }
  ...
  const nextCursor =
    data && data.length === limit ? data[data.length - 1]?.created_at || null : null;

  return res.json({ items: signed, nextCursor });
});

app.get("/api/images/:id", requireAuth, requireActiveProfile, async (req, res) => {
  ...
  if (!signedUrl) {
    return res.status(500).json({ error: "Impossible de générer le lien sécurisé." });
  }
  ...
  try {
    ...
    return res.json({ data: base64, mimeType: data.mime_type || "image/png" });
  } catch (fetchError) {
...
app.delete("/api/images/:id", requireAuth, requireActiveProfile, async (req, res) => {
  const imageId = req.params.id;
  if (!imageId) {
    return res.status(400).json({ error: "Image invalide." });
  }
  ...
  if (data.storage_path) {
    try {
      await supabaseAdmin.storage.from(supabaseBucket).remove([data.storage_path]);
    } catch (e) {
...
app.get("/api/admin/users", requireAuth, requireActiveProfile, requireAdmin, async (req, res) => {
  const { data, error } = await supabaseAdmin
    .from("profiles")
...
app.post("/api/admin/users", requireAuth, requireActiveProfile, requireAdmin, async (req, res) => {
  ...
  if (error || !data?.user) {
    return res.status(400).json({ error: error?.message || "Création impossible." });
  }
  ...
  if (profileError) {
    return res.status(500).json({ error: "Utilisateur créé mais profil manquant." });
  }
...
app.post("/api/generate", requireAuth, requireActiveProfile, async (req, res) => {
  if (!ensureSupabase(res)) return;
  if (!apiKey) {
    return res.status(500).json({ error: "GEMINI_API_KEY manquante dans l'environnement." });
  }

  const { baseImage, inspirationImages, prompt, numImages, imageConfig, mode, projectId } = req.body || {};
  ...
  const inspirations = Array.isArray(inspirationImages)
    ? inspirationImages.filter((image) => image?.data && image?.mimeType).slice(0, 4)
    : [];
  ...
  try {
    ...
    if (missingAfterPrimary > 0 && batch.batchErrors.some(isImageConfigError)) {
    ...
    const storageTasks = images.map(async (image) => {
      const storagePath = await uploadToSupabase({ image, userId: req.user.id });
    ...
    if (images.length < safeCount) {
      warnings.push(`Le modele a retourne ${images.length} image(s) sur ${safeCount}.`);
    }
    if (errors.length > 0) {
...
app.get("/api/projects", requireAuth, requireActiveProfile, async (req, res) => {
  const { data, error } = await supabaseAdmin
    .from("projects")
    .select("id, name, prompt, base_image_storage_path, base_image_mime_type, created_at, updated_at")
...
app.get("/api/projects/:id", requireAuth, requireActiveProfile, async (req, res) => {
...
app.post("/api/projects", requireAuth, requireActiveProfile, async (req, res) => {
  const { name } = req.body || {};
  const safeName = String(name || "Nouveau projet").trim().slice(0, 100);
...
app.patch("/api/projects/:id", requireAuth, requireActiveProfile, async (req, res) => {
  const projectId = req.params.id;
  ...
  if (baseImage && baseImage.data) {
    ...
    if (uploadError) {
      return res.status(500).json({ error: "Impossible d'uploader l'image." });
    }
...
app.delete("/api/projects/:id", requireAuth, requireActiveProfile, async (req, res) => {
  const projectId = req.params.id;
...
app.post("/api/projects/:id/inspirations", requireAuth, requireActiveProfile, async (req, res) => {
  const projectId = req.params.id;
  ...
  if ((existing || []).length >= 4) {
    return res.status(400).json({ error: "Maximum 4 inspirations par projet." });
  }
  ...
  if (uploadError) {
    return res.status(500).json({ error: "Impossible d'uploader l'inspiration." });
  }
  ...
  if (error) {
    return res.status(500).json({ error: "Impossible d'ajouter l'inspiration." });
  }
...
app.delete(
  ...
  async (req, res) => {
    ...
    if (inspiration.storage_path) {
      try {
        await supabaseAdmin.storage.from(supabaseBucket).remove([inspiration.storage_path]);
      } catch (e) {
        // Ignorer les erreurs
      }
    }
